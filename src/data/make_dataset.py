...
def get_train_test_datasets(data_path: str, test_size: float = 0.2, window_size: int = 100, validation_size: float = None, 
    ...
    Args:
        ...
        forcasting_horizon (int, optional): forcasting horizon. Defaults to 10.
...
class sliding_widow_dataset(Dataset):
    ...
    def __init__(self, data: pd.DataFrame, window_size: int = 100, 
                 sliding_size: int = 1, forcasting_horizon: int = 10):
        """
        ...
        self.nb_of_sequences = (self.data_size - self.window_size - self.forcasting_horizon) // self.sliding_size + 1

    def __len__(self):
        return (self.data_size - self.window_size - self.forcasting_horizon) // self.sliding_size + 1
    ...
    def __getitem__(self, idx):
        ...
        y = self.data.iloc[idx+self.window_size:idx+self.window_size+self.forcasting_horizon].values
